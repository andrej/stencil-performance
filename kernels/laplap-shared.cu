...
void laplap_shared(GRID_ARGS const coord3 halo, const coord3 max_coord, const value_t *in, value_t *out) {
    ...
    if(threadIdx.x == 0) {
        ...
        lap_left = local_lap[SMEM_NEIGHBOR_OF_INDEX(s_center, -1, 0, 0)];
    }
    ...
    if(threadIdx.x == blockDim.x-1 || i == max_coord.x-1) {
        ...
        lap_right = local_lap[SMEM_NEIGHBOR_OF_INDEX(s_center, +1, 0, 0)];
    }
    ...
    if(threadIdx.y == 0) {
        ...
        lap_right = local_lap[SMEM_NEIGHBOR_OF_INDEX(s_center, 0, -1, 0)];
    }
