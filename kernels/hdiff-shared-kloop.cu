...
void hdiff_shared_kloop(const HdiffCudaBase::Info info,
    ...
    for(int k = info.halo.z; k < info.max_coord.z; k++) {
        ...
        const value_t lap_ij = 4 * in[NEIGHBOR(i, j, k, 0, 0, 0)] 
            - in[NEIGHBOR(i, j, k, -1, 0, 0)] - in[NEIGHBOR(i, j, k, +1, 0, 0)]
            - in[NEIGHBOR(i, j, k, 0, -1, 0)] - in[NEIGHBOR(i, j, k, 0, +1, 0)];
        local_lap[SMEM_INDEX((int)threadIdx.x, (int)threadIdx.y, 0)] = lap_ij;
        ...
        if(threadIdx.x == blockDim.x-1 || i == info.max_coord.x-1) {
            ...
            lap_ipj = local_lap[SMEM_NEIGHBOR((int)threadIdx.x, (int)threadIdx.y, 0, +1, 0, 0)];
        }
        ...
        if(threadIdx.x == 0) {
            ...
            flx_imj = local_flx[SMEM_NEIGHBOR((int)threadIdx.x, (int)threadIdx.y, 0, -1, 0, 0)];
        }
        ...
        if(threadIdx.y == 0) {
            ...
            fly_ijm = local_fly[SMEM_NEIGHBOR((int)threadIdx.x, (int)threadIdx.y, 0, 0, -1, 0)];
        }
